...
`tests/test_text_exporter.py`/`tests/test_docx_exporter.py`. Currently 128
tests total (127 fast + the same 1 gated test) after later sessions'
additions, most recently `tests/test_searchable_pdf_exporter.py` (8 tests,
see the `SearchablePDFExporter` entry above) and
`tests/test_low_confidence_flagger.py` plus new cases in
`tests/test_text_exporter.py`/`tests/test_docx_exporter.py` (see the
`LowConfidenceFlagger` entry above).
...
- Searchable-PDF export (opt-in checkbox, off by default, added 2026-07-30) - see the `SearchablePDFExporter` entry above; participates in the same checkpoint/resume mechanism as TXT/DOCX
- Low-confidence word flagging (automatic, no checkbox - added 2026-07-30, same "no reason to disable it" reasoning as heading detection) - see the `LowConfidenceFlagger` entry above; visible bracket markers in `.txt`, yellow highlight in `.docx`

**Not built / explicitly out of scope** (per Section 7 and the minimal-GUI
framing in the file's own docstring): JSON export, job queues beyond simple
sequential folder processing, any preprocessing (deskew/denoise/CLAHE -
measured to hurt Google Vision's accuracy, see the `GoogleVisionEngine`
entry), and any local/offline engine as the default (PaddleOCR and Qaari
both disqualified, then removed from the codebase entirely on 2026-07-26 -
see the engine comparison table and Layer 6 removal notes above).
